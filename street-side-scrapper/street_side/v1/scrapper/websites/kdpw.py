...
def iosco_filter(link) -> bool:
    for content in link.contents:
        try:
            ...
            if (
                "Quantitative Disclosure" in elements_text
                and
                "KDPW_CCP" in elements_text
            ):
                return True
        except:
            continue
...
def find_and_filter_links(web_page: WebPage) -> Tuple[
    ...
    scrapped_company = Company(
        short_name="KDPW CPP",
        full_name="Krajowy Depozyt Papierów Wartościowych (KDWP)",
        home_url=home_url,
        created_at=None,
        updated_at=None
    )

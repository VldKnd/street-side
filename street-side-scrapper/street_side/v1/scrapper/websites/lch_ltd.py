...
def pfmi_sa_filter(link) -> bool:
    for content in link.contents:
        try:
            if (
                "Qualitative Assessment" in content
                and
                "IOSCO" in content
            ):
                return True
...
def find_and_filter_links(web_page: WebPage) -> Tuple[
    ...
    scrapped_company = Company(
        short_name="LCH LTD",
        full_name="London Clearing House LTD",
        home_url=home_url,
        created_at=None
    )
    ...
    for pfmi_sa_link in pfmi_sa_links:
        ...
        if pfmi_sa_link.string is not None:
            ...
            for content in pfmi_sa_link.contents:
                try:
                    year_regex = re.search(r'\d{4}', content.string)
                    if year_regex is not None:
                        year = year_regex.group()
                        published_date = datetime(int(year), 12, 1)
                except:
